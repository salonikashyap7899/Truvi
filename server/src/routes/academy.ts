...
router.get("/progress", async (req: AuthedRequest, res) => {
  const db = getDb();
  const progress = await db.select().from(courseProgress).where(eq(courseProgress.userId, req.user!.userId));
  res.json({ progress });
});
...
router.delete("/progress/:courseId/module/:moduleId", async (req: AuthedRequest, res) => {
  ...
  const [updated] = await db
    .update(courseProgress)
    .set({
      completedModules: progress.completedModules.filter((m) => m !== moduleId),
      completedAt: null,
    })
    .where(eq(courseProgress._id, progress._id))
    .returning();
