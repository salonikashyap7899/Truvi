...
import { users, projects, notifications, type Role, type ApprovalStatus, type VerificationDetails, DEFAULT_VERIFICATION_DETAILS } from "../db/schema";
...
router.get("/users", requireRole("ADMIN"), async (req, res) => {
  ...
  if (typeof role === "string" && role) {
    conds.push(eq(users.role, role as Role));
  } else {
    conds.push(inArray(users.role, ["DEVELOPER", "CP", "BUYER"]));
  }
  if (typeof approvalStatus === "string" && approvalStatus) {
    conds.push(eq(users.approvalStatus, approvalStatus as ApprovalStatus));
  }
...
router.get("/projects", requireRole("ADMIN"), async (req, res) => {
  ...
  const conds = [];
  if (typeof approvalStatus === "string" && approvalStatus) {
    conds.push(eq(projects.approvalStatus, approvalStatus as ApprovalStatus));
  }
