...
async function seed() {
  ...
  if (!url) {
    throw new Error("DATABASE_URL is not set. Add your Supabase Postgres connection string to server/.env before seeding.");
  }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let u = 0; u < unitCount; u++) {
      ...
      unitValues.push({
        ...
        price,
        status: status as "AVAILABLE" | "SOLD" | "RESERVED" | "LOCKED",
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const [lead] = await db
      ...
      .values({
        ...
        clientEmail: Math.random() > 0.5 ? `client${i}@example.com` : null,
        stage,
  ...
  for (const lead of bookingLeads) {
    ...
    const built = buildMilestones(calc.cpCommissionAmount);
    const releaseCount = Math.floor(Math.random() * 4);

    const milestones: CommissionMilestone[] = built.map((m, idx) => ({
      _id: randomUUID(),
      label: m.label,
    ...
    await db.insert(commissions).values({
      ...
      status: releaseCount === 0 ? "PENDING" : releaseCount === built.length ? "PAID" : "MILESTONE_DUE",
      milestones,
    });
  }
