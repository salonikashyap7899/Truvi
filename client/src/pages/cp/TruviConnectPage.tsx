...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  DISCUSSION: "bg-neutral-800/60 text-neutral-300 border-neutral-700",
...
export default function TruviConnectPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0B1220] p-6 text-white md:p-10 pb-20">
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-0.5">Community for CPs, Developers, and the Truvi team.</p>
        </div>
      </div>

      {/* Compose */}
      <div className="rounded-2xl border border-neutral-800 bg-[#121A2B] p-4 mb-6">
        <p className="text-xs font-medium text-neutral-400 mb-2">Share with the community</p>
        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          placeholder="Share a tip, market update, or start a discussion…"
          className="w-full bg-[#0f1624] border border-neutral-700 rounded-xl p-3 text-sm text-white placeholder:text-neutral-600 resize-none outline-none focus:border-purple-600 transition-colors"
          rows={3}
        />
        <div className="mt-3 flex items-center justify-between gap-3">
          <select
            ...
            className="bg-[#0f1624] border border-neutral-700 rounded-lg px-3 py-1.5 text-xs text-white outline-none"
          >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
        {FILTERS.map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`shrink-0 rounded-full px-4 py-1.5 text-xs font-medium transition-colors ${filter === f ? "bg-purple-600 text-white" : "bg-[#121A2B] border border-neutral-800 text-neutral-400 hover:text-white"}`}
          >
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-neutral-800 bg-[#121A2B] p-10 text-center">
          <p className="text-neutral-500 text-sm">No posts yet. Be the first to share!</p>
        </div>
      ) : (
        <div className="space-y-4">
          {posts.map((post) => (
            <div key={post._id} className="rounded-2xl border border-neutral-800 bg-[#121A2B] p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className={`text-[10px] rounded px-1.5 py-0.5 ${ROLE_BADGE[post.authorRole] || "bg-neutral-800 text-neutral-400"}`}>
                        {post.authorRole}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="text-[10px] text-neutral-600">{formatDate(post.createdAt)}</span>
                </div>
              </div>
              <p className="mt-3 text-sm text-neutral-200 leading-relaxed whitespace-pre-wrap">{post.content}</p>
              <div className="mt-3 flex items-center gap-3 border-t border-neutral-800 pt-3">
                <button
                  onClick={() => toggleLike(post._id)}
                  className={`flex items-center gap-1.5 text-xs transition-colors ${likedPosts.has(post._id) ? "text-rose-400" : "text-neutral-500 hover:text-rose-400"}`}
                >
