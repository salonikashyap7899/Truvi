...
import {
  ...
  BadgePercent, Activity, ShieldCheck,
} from "lucide-react";
...
/* ------------------------------------------------------------------ */
/* Developer Referral — onboard a developer, earn a one-time first-txn */
/* bonus (₹75 for a partner) + 2% lifetime on their every transaction. */
/* ------------------------------------------------------------------ */
interface DevRefRow {
  _id: string;
  name: string;
...
function DeveloperReferralSection() {
  ...
  return (
    <Card className="mt-8 border-sky-500/25 bg-sky-950/10 text-white">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
      ...
      {/* Summary */}
      <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
        <AmbTile icon={<Users size={15} />} label="Referred Developers" value={String(s.referredCount)} />
        <AmbTile icon={<ShieldCheck size={15} />} label="Active" value={String(s.active)} tone="text-emerald-300" />
        <AmbTile icon={<Activity size={15} />} label="Transactions" value={String(s.totalTransactions)} />
        <AmbTile icon={<IndianRupee size={15} />} label="1st-txn Bonus" value={formatINR(s.totalBonus)} tone="text-amber-300" />
        <AmbTile icon={<TrendingUp size={15} />} label="Total Earnings" value={formatINR(s.totalEarnings)} tone="text-emerald-300" />
      </div>
