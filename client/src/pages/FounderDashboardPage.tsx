...
const Ic = ({ n }: { n: string }) => P(ICONS[n] || ICONS.grid);

/* --------------------------------------------------------------- helpers */
type Tone = "blue" | "green" | "amber" | "red";
function Kpi({ icon, tone, label, value, foot, trend }: { icon: string; tone: Tone; label: string; value: string; foot?: string; trend?: { text: string; up?: boolean } }) {
  return (
...
/* =================================================================== page */
type Page = "overview" | "sales" | "projects" | "crm" | "finance" | "verification" | "kpi" | "insights" | "team" | "marketing" | "land" | "investor";
...
export default function FounderDashboardPage() {
  ...
  if (loading) return <div className="founder-os" style={{ padding: 40 }}><p style={{ color: "var(--ink-500)" }}>Loading founder command center…</p></div>;
  if (!d) return <div className="founder-os" style={{ padding: 40 }}><p>Could not load the dashboard. Please retry.</p></div>;

  const nav: { group: string; items: { key: Page; label: string; icon: string; count?: number }[] }[] = [
      ...
      <aside className={`sidebar ${navOpen ? "open" : ""}`}>
        <div className="brand">
          ...
          <div>
            ...
            <div className="brand-sub">CEO OS</div>
          </div>
        </div>
        <nav className="nav-scroll">
          {nav.map((g) => (
            <div key={g.group}>
              ...
              {g.items.map((it) => (
                <button key={it.key} className={`nav-item ${page === it.key ? "active" : ""}`} onClick={() => go(it.key)}>
                  <Ic n={it.icon} />
                  <span>{it.label}</span>
        ...
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <button className="profile-btn">
              <div className="avatar">{initials(user?.name || "Founder")}</div>
              <div><div className="profile-name">{user?.name || "Founder"}</div><div className="profile-role">Founder</div></div>
            </button>
          </div>
        </header>

        <div className="content">
          {page === "overview" && <OverviewPage d={d} go={go} />}
          {page === "sales" && <SalesPage d={d} />}
          {page === "projects" && <ProjectsPage d={d} />}
          {page === "crm" && <CrmPage d={d} />}
          {page === "finance" && <FinancePage fin={fin} navigate={navigate} />}
          {page === "verification" && <VerificationPage d={d} />}
          {page === "kpi" && <KpiPage d={d} fin={fin} />}
          {page === "insights" && <InsightsPage d={d} fin={fin} />}
          {page === "team" && <Placeholder title="Team Dashboard" what="Employees, attendance, productivity, hiring and performance ranking need an HR module." connect="Add an employees/attendance schema or connect an HRMS." />}
          {page === "marketing" && <Placeholder title="Marketing Dashboard" what="Website traffic, social growth, ad spend, cost-per-lead and campaign ROI need analytics + ad-platform integrations." connect="Connect GA4 and Meta/Google Ads, or add a campaigns table." />}
          {page === "land" && <Placeholder title="Land Bank" what="Total land, verified land, acquisition pipeline, valuation and due-diligence need a land-parcel register." connect="Add a land-parcels table (area, ownership, DD status, valuation)." />}
          {page === "investor" && <Placeholder title="Investor Dashboard" what="Valuation, fundraising, cap table and ESOP pool are founder-confidential and need a cap-table source." connect="Add a cap-table/fundraise schema. Valuation-driving KPIs are already live under KPIs." />}
        </div>
      </div>
...
function OverviewPage({ d, go }: { d: Overview; go: (p: Page) => void }) {
  const ex = d.executive;
  return (
    <section className="page">
      <div className="page-header">
        <div><div className="page-title">Founder Command Center</div><div className="page-sub">Real-time snapshot across revenue, growth, trust &amp; operations · Live data</div></div>
...
function ProjectsPage({ d }: { d: Overview }) {
  return (
    <section className="page">
      <div className="page-header"><div><div className="page-title">Projects</div><div className="page-sub">Listings, approval &amp; verification status</div></div></div>
      <div className="kpi-grid">
...
function VerificationPage({ d }: { d: Overview }) {
  return (
    <section className="page">
      <div className="page-header"><div><div className="page-title">Verification &amp; Legal</div><div className="page-sub">Trust queues across projects, KYC &amp; legal docs</div></div></div>
      <div className="kpi-grid">
